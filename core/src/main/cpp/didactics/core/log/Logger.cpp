...
Logger::Logger(const std::string & logFileProperties,std::string path)
{
...
bool Logger::initLogger()
{
    ...
    else
    {
        ...
        if (configurationPath.empty() /*&& getenv(log4cxx.properties)*/) 
        {
            ...
            log4cxx::PatternLayoutPtr   layout   (new log4cxx::PatternLayout  ("%d{yyyy-MM-dd HH:mm:ss.SSS} (%-6c) [%-6p] [%15.15t] (%-10.20l) -- %m%n"));
...
std::string 
Logger::getFileExtension(const std::string& s) 
{
    LOG4CXX_TRACE(LOG, __LOG4CXX_FUNC__);        
...
void 
Logger::setLoggerLevel(const std::string &loggerId, const std::string &level)
{
    LOG4CXX_TRACE(LOG, __LOG4CXX_FUNC__);        
...
void 
Logger::setRootDefaultLevel(const std::string &level)
{
    LOG4CXX_TRACE(LOG, __LOG4CXX_FUNC__);
